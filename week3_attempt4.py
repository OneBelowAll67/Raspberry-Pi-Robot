...
def main():
    ...
    ENA, IN1, IN2, ENB, IN3, IN4 = 12, 23, 22, 5, 17, 24
    
    for pin in [ENA, IN1, IN2, ENB, IN3, IN4]:
        GPIO.setup(pin, GPIO.OUT)
        
    pwmA = GPIO.PWM(ENA, 700)
    pwmB = GPIO.PWM(ENB, 700)
    pwmA.start(0)
    pwmB.start(0)
    ...
    picam2.set_controls({"ExposureTime": 15000, "AnalogueGain": 2.5})
    ...
    try:
        while True:
                ...
                if action in action_cooldowns and \
                   (current_time - action_cooldowns[action]) < COOLDOWN_DURATION:
                    ...
                    if action == "HALT_3_SEC":
                        ...
                        # INVERSE MEMORY: Turn Left -> Save Right for exit
                        junction_memory = "RIGHT"  
                        set_motor(80, -50)
                        time.sleep(0.4)

                    elif action == "TURN_RIGHT_90":
                        # INVERSE MEMORY: Turn Right -> Save Left for exit
            ...
            # ---------------------------------------------------------
            # PERIPHERAL VISION TRIPWIRES (WITH MEMORY FIX)
            # ---------------------------------------------------------
            ...
            left_trigger  = (left_red_pixels > TRIPWIRE_THRESHOLD or left_yel_pixels > TRIPWIRE_THRESHOLD)
            right_trigger = (right_red_pixels > TRIPWIRE_THRESHOLD or right_yel_pixels > TRIPWIRE_THRESHOLD)

            if prev_tracked_color == "BLACK":
                
                # 1. EXCLUSIVE MEMORY OVERRIDE (Handles skewed exits perfectly)
                if junction_memory and (left_trigger or right_trigger):
                    ...
                    junction_memory = None # Wipe memory after a successful exit
            ...
            if contours:
                ...
                if valid_to_drive:
                    ...
                    if M["m00"] != 0:
                        ...
                        DYNAMIC_MAX_SPEED = 20 if active_color in ("RED", "YELLOW") \
