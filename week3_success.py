...
# ══════════════════════════════════════════════════════════════
IN1, IN2 = 17, 24   # motor_a (Left) direction
IN3, IN4 = 23, 22  # motor_b (Right) direction
ENA, ENB = 13, 12   # PWM enable
...
# SPEED / PID CONSTANTS
# ══════════════════════════════════════════════════════════════
MOTOR_A_SPEED_NORMAL,  MOTOR_B_SPEED_NORMAL  = 25, 25
MOTOR_A_SPEED_PUSH,    MOTOR_B_SPEED_PUSH    = 28, 28

KP, KI, KD         = 0.23, 0.010, 0.0315
X_CENTRE_REF        = 240
Y_CENTRE_REF        = 180
...
SAMPLE_DICT = {
    ...
    2: (["qr.png"],        10),
    3: (["recycle.png"], 5),
    4: (["warning.png"],  15) # Lowered for motion blur
}
        ...
        while True:
            ...
            if active_instruction:
                if active_instruction == "TURN_LEFT":
                    ...
                    move_forward(pwm_motor_a, pwm_motor_b, -70, 70); time.sleep(2.9)
                    stop_motors(pwm_motor_a, pwm_motor_b); active_instruction = ""
            else:
                if current_is_priority:
