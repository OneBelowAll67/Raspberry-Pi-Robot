...
def main():
    ...
    try:
        while True:
                ...
                if action in action_cooldowns and \
                   (current_time - action_cooldowns[action]) < COOLDOWN_DURATION:
                    ...
                    if action == "HALT_3_SEC":
                        ...
                        print("[FSM] Initiating Progressive Sweep Search...")
                        set_motor(0, 0)
                        time.sleep(0.2)
                        sweep_durations  = [0.4, 0.8, 1.2, 1.6]
                        sweep_directions = [1, -1, 1, -1]
                        line_found = False

                        for i in range(len(sweep_durations)):
                            if line_found: break
                            if sweep_directions[i] == 1: set_motor(50, -50)
                            else:                        set_motor(-50, 50)

                            timeout = time.time() + sweep_durations[i]
                            while time.time() < timeout:
                                spin_frame = picam2.capture_array()
                                spin_roi   = spin_frame[90:240, :]
                                spin_gray  = cv2.cvtColor(spin_roi, cv2.COLOR_RGB2GRAY)
                                _, spin_thresh = cv2.threshold(
                                    cv2.GaussianBlur(spin_gray, (5, 5), 0),
                                    80, 255, cv2.THRESH_BINARY_INV)
                                spin_contours, _ = cv2.findContours(
                                    spin_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                                if spin_contours:
                                    c = max(spin_contours, key=cv2.contourArea)
                                    if cv2.contourArea(c) > 200:
                                        M = cv2.moments(c)
                                        if M["m00"] != 0:
                                            spin_cx = int(M["m10"] / M["m00"])
                                            if abs(spin_cx - CENTER) < 50:
                                                set_motor(0, 0)
                                                line_found = True
                                                break
            ...
            # ---------------------------------------------------------
            # FILTER BY AREA AND WIDTH (IGNORE WIDE SIGNS)
            # ---------------------------------------------------------
            MAX_LINE_WIDTH = 50  # Tuned lower for the screenshot resolution

            # --- TEMPORARY DEBUG CODE ---
            # Watch your terminal! This prints the width of every yellow block
            for c in c_yel:
                if cv2.contourArea(c) > 80:
                    w = cv2.boundingRect(c)[2]
                    print(f"[DEBUG] Saw a yellow shape with Width: {w}")
            # ----------------------------

            valid_red = [c for c in c_red if cv2.contourArea(c) > 80 and cv2.boundingRect(c)[2] < MAX_LINE_WIDTH]
            valid_yel = [c for c in c_yel if cv2.contourArea(c) > 80 and cv2.boundingRect(c)[2] < MAX_LINE_WIDTH]
